...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.error('   If you are deploying to a local node, start it with:');
      console.error('     npx hardhat node');
      console.error('   Or run this script without `--network localhost` to use Hardhat\'s');
      console.error('   built-in ephemeral network:');
      console.error('     npx hardhat run scripts/deploy-ftso-reader.js');
