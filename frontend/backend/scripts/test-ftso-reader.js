...
async function main() {
  ...
  try {
    let tester;
    try {
      [tester] = await hre.ethers.getSigners();
    } catch (err) {
      console.error('\n❌ Could not connect to the configured network.');
      console.error('   If you are testing against a local node, start it with:');
      console.error('     npx hardhat node');
      console.error('   Or run this test script without `--network localhost` to use Hardhat\'s');
      console.error('   built-in ephemeral network:');
      console.error('     npx hardhat run scripts/test-ftso-reader.js');
      console.error('\n   Error details:', err.message.split('\n')[0]);
      process.exit(1);
    }
    console.log("\n📝 Test Information:");
